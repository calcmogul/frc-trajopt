...
class UnicycleTrajectoryOptimizer:
    ...
    def optimize(self, q: float, r: List[float]):
        ...
        # States: [x, y, heading]
        X = self.opti.variable(3, num_vars + 1)
        ...
        self.opti.subject_to(U[:, 0] == np.zeros((2, 1)))
        self.opti.subject_to(U[:, -1] == np.zeros((2, 1)))
        ...
        self.opti.subject_to(
            self.opti.bounded(-v_max, U[0, :] - U[1, :] * self.trackwidth, v_max)
        )
        self.opti.subject_to(
            self.opti.bounded(-v_max, U[0, :] + U[1, :] * self.trackwidth, v_max)
        )
        self.opti.subject_to(self.opti.bounded(-w_max, U[1, :], w_max))
