...
class UnicycleTrajectoryOptimizer:
    class Waypoint:
        def __init__(
            self,
            x: float,
    ...
    def __init__(self, trackwidth: float, dt: float, initial_pose: Pose2d) -> None:
        """
        Constructs a unicycle trajectory optimizer.

        Args:
            trackwidth: Differential drive trackwidth.
            dt: The sample period.
            initial_pose: The unicycle's initial pose.
        """
        ...
        self.waypoints = [
            UnicycleTrajectoryOptimizer.Waypoint(
    ...
    def add_constraint(self, constraint: TrajectoryConstraint) -> None:
        ...
        Args:
            constraint: Tthe constraint to apply.
        """
    ...
    def optimize(self, q: float, r: list[float]):
        ...
        # States: [x, y, heading]
        X = self.problem.variable(3, num_vars + 1)
        ...
        for i, waypoint in enumerate(self.waypoints):
            ...
            if waypoint.heading is not None:
                self.problem.subject_to(X[2, segment_end] == waypoint.heading)
            if i > 0:
        ...
        self.problem.subject_to(U[:, 0] == np.zeros((2, 1)))
        self.problem.subject_to(U[:, -1] == np.zeros((2, 1)))
        ...
        self.problem.subject_to(
            self.problem.bounded(-v_max, U[0, :] - U[1, :] * self.trackwidth, v_max)
        )
        self.problem.subject_to(
            self.problem.bounded(-v_max, U[0, :] + U[1, :] * self.trackwidth, v_max)
        )
        self.problem.subject_to(self.problem.bounded(-w_max, U[1, :], w_max))
        ...
        return UnicycleTrajectoryOptimizer.resample(
    ...
    def f(
        ...
        """
        The dynamical model for a unicycle.

        Args:
            x: State column vector: [x, y, heading].
            u: Input column vector: [velocity, angular velocity].
